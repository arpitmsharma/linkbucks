			...
			.ready(
					function() {
						if ($.fn.plot) {
							...
							var d1 = [ [ 0, 10 ], [ 1, 20 ], [ 2, 33 ],
									[ 3, 24 ], [ 4, 45 ], [ 5, 96 ], [ 6, 47 ],
									[ 7, 18 ], [ 8, 11 ], [ 9, 13 ], [ 10, 21 ]
							...
							var plot = $.plot($("#daily-visit-chart"), data,
									options);
						...
						// Jquery vector map
						if ($.fn.vectorMap) {
							var cityAreaData = [ 500.70, 410.16, 210.69,
									120.17, 64.31, 150.35, 130.22, 120.71,
									300.32 ]
							$('#world-map').vectorMap({
								map : 'us_lcc_en',
												...
												if (p == 13) {
													if (inText == "") {
														...
														$(
																'<li>'
																		+ inText
																		+ '<a href="#" class="event-close"> <i class="ico-close2"></i> </a> </li>')
																.appendTo(
																		'.event-list');
													}
													$(this).val('');
						...
						$(function() {
							$('.chat-input')
									.keypress(
											function(ev) {
												var p = ev.which;
												var chatTime = moment().format(
														"h:mm");
												var chatText = $('.chat-input')
														.val();
												if (p == 13) {
													if (chatText == "") {
														alert('Empty Field');
														...
														$(
																'<li class="clearfix"><div class="chat-avatar"><img src="images/chat-user-thumb.png" alt="male"><i>'
																		+ chatTime
																		+ '</i></div><div class="conversation-text"><div class="ctext-wrap"><i>John Carry</i><p>'
																		+ chatText
																		+ '</p></div></div></li>')
																.appendTo(
																		'.conversation-list');
...
if (Gauge) {
	...
	var target = document.getElementById('gaugeUserLink'); // your canvas
	// element
	...
	var target1 = document.getElementById('gaugeOtherLink'); // your canvas
	// element
	...
	gauge1.set(1150); // set actual value
	gauge1.setTextField(document.getElementById("gauge-textfield"));
